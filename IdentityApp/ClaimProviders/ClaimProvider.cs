...
namespace IdentityApp.ClaimProviders
{
    public class ClaimProvider : IClaimsTransformation
    {
        ...
        {
            if(principal!=null && principal.Identity.IsAuthenticated)// if user is registerd in system
            {
                ...
                if (user != null)
                {
                    ...
                    if(user.City!=null)
                    {
                        if(!principal.HasClaim(c=>c.Type=="city")) //control if there is not any claims with type city
                        {
